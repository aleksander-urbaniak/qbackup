          ...
          <div className="min-h-0 flex-1 overflow-auto">
            {rows.map((entry) => (
              <button
                ...
                className={`grid w-full grid-cols-[minmax(0,1fr)_88px_132px] items-center gap-3 border-b border-slate-800/50 px-4 py-2.5 text-left text-sm hover:bg-slate-800/60 ${selectedEntry?.path === entry.path ? 'bg-blue-600/25 text-white' : openFile === entry.path ? 'bg-blue-600/10 text-white' : 'text-slate-300'}`}
              >
                <span className="flex min-w-0 items-center gap-2" style={{ paddingLeft: `${entry.depth * 18}px` }}>
                  ...
                  {entry.type === 'directory' ? <Folder className="h-4 w-4 shrink-0 text-amber-300" /> : <File className="h-4 w-4 shrink-0 text-slate-400" />}
        ...
        <section className="flex min-h-[520px] flex-col overflow-hidden rounded-lg border border-slate-800/80 bg-[#0B101A]">
          <div className="flex items-center justify-between gap-3 border-b border-slate-800/80 px-4 py-3">
            <div className="min-w-0">
              <div className="truncate text-sm font-medium text-slate-200">{openFile || 'No file open'}</div>
              <div className="text-xs text-slate-500">{dirty ? 'Unsaved changes' : openFile ? 'Saved' : 'Open a text file from the explorer'}</div>
            </div>
            <button
              type="button"
